...
app.get('/download/:token', (req, res) => {
    ...
    if (!token) return res.status(404).send('Invalid download token.');

    db.query(
        `SELECT r.file_name, r.file_type, r.allow_download, r.is_paid, d.token_used, d.token_expires_at, d.payment_status, d.resource_id
         FROM resource_purchase_sessions d
         JOIN learning_resources r ON r.id = d.resource_id
         WHERE d.download_token = ? LIMIT 1`,
        [token],
        (err, rows) => {
            if (err) return res.status(500).send('Server error.');
            if (!rows || rows.length === 0) return res.status(404).send('Download token not found.');
            const row = rows[0];
            if (String(row.payment_status) !== 'PAID') return res.status(403).send('Payment not completed.');
            if (!row.allow_download) return res.status(403).send('Download not permitted.');
            const now = new Date();
            const expiresAt = row.token_expires_at ? new Date(row.token_expires_at) : null;
            if (!expiresAt || expiresAt < now) {
                return res.status(410).send('This download token has expired.');
            }
            if (Number(row.token_used || 0) === 1) {
                return res.status(410).send('This purchase has already been completed.');
            }

            const filePath = row.is_paid === 1
                ? path.join(__dirname, 'protected', 'uploads', row.file_name)
                : path.join(__dirname, 'public', 'assets', 'uploads', row.file_name);

            fs.stat(filePath, (statErr, stats) => {
                if (statErr || !stats.isFile()) return res.status(404).send('File not found.');

                db.query(
                    `UPDATE resource_purchase_sessions SET token_used = 1, token_used_at = NOW() WHERE download_token = ? AND token_used = 0 AND token_expires_at >= NOW()`,
                    [token],
                    (updateErr, updateResult) => {
                        if (updateErr) return res.status(500).send('Server error.');
                        ...
                        stream.on('error', () => res.status(500).send('File stream error.'));
                        stream.pipe(res);
                    }
                );
            });
...
app.use('/assets/uploads', (req, res, next) => {
    ...
    db.query(
        `SELECT COUNT(*) AS count FROM learning_resources WHERE file_name = ? AND is_paid = 1`,
        [requestedFile],
        (err, rows) => {
            if (!err && rows && rows[0] && Number(rows[0].count || 0) > 0) {
                return res.status(403).send('Direct access to paid resources is forbidden.');
            }
            next();
        }
    );
